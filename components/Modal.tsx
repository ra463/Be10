...
const Modal = ({ setOpen, getData }: { setOpen: any; getData: any }) => {
  const [message, setMessage] = useState("");
  const [loading, setLoading] = useState(false);

  const handleAddInfo = async (e: any) => {
    e.preventDefault();
    setLoading(true);

    const { data } = await axios.post("/api/info/addInfo", {
      message,
    });
